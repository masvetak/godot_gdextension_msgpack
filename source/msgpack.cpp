...
Variant Msgpack::_unpack(StreamPeerBuffer &buffer, Dictionary &error) { // NOLINT(*-no-recursion)
...
PackedByteArray Msgpack::pack(const Variant& data) {
    pack_buffer.clear();
    pack_buffer.set_big_endian(true);

    Dictionary error;
    error["error"] = Error::OK;
    error["error_message"] = "";

    _pack(data, pack_buffer, error);

    if (int(error["error"]) != Error::OK) {
        UtilityFunctions::print(error["error_message"]);
    }
    return pack_buffer.get_data_array();
}

Variant Msgpack::unpack(const PackedByteArray& data) {
    unpack_buffer.clear();
    unpack_buffer.set_big_endian(true);
    unpack_buffer.set_data_array(data);

    Dictionary error;
    error["error"] = Error::OK;
    error["error_message"] = "";

    Variant result = _unpack(unpack_buffer, error);
